document.addEventListener('DOMContentLoaded', function () {
    ...
    if (registerSubmitBtn) {
        registerSubmitBtn.addEventListener('click', function (event) {
            ...
            // Send registration request to server using GET with query parameters
            fetch(`http://${IP}:8081/register?username=${encodeURIComponent(username)}&email=${encodeURIComponent(email)}&password=${encodeURIComponent(password)}`, {
    ...
    if (loginSubmitBtn) {
        loginSubmitBtn.addEventListener('click', function (event) {
            ...
            // Send login request to server using GET with query parameters
            fetch(`http://${IP}:8081/login?username=${encodeURIComponent(username)}&password=${encodeURIComponent(password)}`, {
                method: 'GET', // Change method to GET
            })
                .then(response => {
                    if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
